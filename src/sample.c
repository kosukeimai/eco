...
void rMHrc(
	   double **W,             /* W_i */
	   double *Wsum,           /* sum of W_i for each column */
	   double *X,              /* X_i */
	   double Y,               /* Y_ij */
	   double *minU,           /* lower bound for U */
	   double *maxU,           /* upper bound for U */
				   ...
				   */  
{
  int i, j, k, exceed;
  double dens1, dens2, ratio, dtemp;
  double *SampleW = doubleArray(n_col);
  double *SampleWsum = doubleArray(n_col);
  double **Wstar = doubleMatrix(n_col, n_dim);
  double **SampleWstar = doubleMatrix(n_col, n_dim);
  double *param = doubleArray(n_col);
  double *U = doubleArray(n_col);
  ...
  if (reject) { /* rejection sampling */
    i = 0; exceed = 1;
    while (exceed > 0) {
      rDirich(U, param, n_col);
      exceed = 0;
      for (k = 0; k < n_col; k++) 
	if (U[k] > maxU[k] || U[k] < minU[k])
	  exceed++;
      i++;
      if (i > maxit)
  ...
  for (k = 0; k < n_col; k++) {
    SampleW[k] = U[k]*Y/X[k];
    SampleWsum[k] = Wsum[k]-W[row][k]+SampleW[k];
  }
  for (j = 0; j < n_dim; j++)
    for (k = 0; k < n_col; k++) {
      if (j == row)
	SampleWstar[k][j] = log(SampleW[k])-log(1-SampleWsum[k]);
      else
	SampleWstar[k][j] = log(W[j][k])-log(1-SampleWsum[k]);
      Wstar[k][j] = log(W[j][k])-log(1-Wsum[k]);
    }
  /* acceptance ratio */
  dens1 = 0; dens2 = 0;
  for (k= 0; k < n_col; k++) {
    dens1 += dMVN(SampleWstar[k], mu[k], InvSigma[k], n_dim, 1);
    dens2 += dMVN(Wstar[k], mu[k], InvSigma[k], n_dim, 1);
    dens1 -= log(SampleWstar[k][row]);
    dens2 -= log(Wstar[k][row]);
  }
  ratio = fmin2(1, exp(dens1-dens2));
  ...
  FreeMatrix(SampleWstar, n_col);
  FreeMatrix(Wstar, n_col);
  free(SampleW);
  free(SampleWstar);
  free(param);
  free(U);
