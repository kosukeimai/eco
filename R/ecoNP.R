ecoNP <- function(formula, data = parent.frame(), supplement = NULL,
  ...
  res.out <- list(call = mf, X = X, Y = Y, W1 = W1.post, W2 = W2.post,
  ...
  if (parameter){
    ...
    res.out$mu <- array(rbind(mu1.post, mu2.post), c(n.a, 2, unit.par))
    res.out$Sigma <- array(rbind(Sigma11.post, Sigma12.post,
                                 Sigma22.post), c(n.a, 3, unit.par))
    if (alpha.update)
      res.out$alpha <- matrix(res$pdSa, n.a, unit.a, byrow=TRUE)
    else
      res.out$alpha <- alpha
    res.out$nstar <- matrix(res$pdSn, n.a, unit.a, byrow=TRUE)
  }
